    ...
    try {
      ...
      for (const channel of channels) {
        switch (channel) {
          case "in_app":
            results.push(
              await this.sendInAppNotification(userId, type, title, message),
            );
            break;
          case "email":
            results.push(
      ...
      this.logger.error(
        `Failed to send notifications for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  private async sendEmailNotification(
    ...
    try {
      ...
      const userResponse = await axios.get(
        `${process.env.AUTH_SERVICE_URL || "http://localhost:3001"}/api/auth/profile`,
        {
      ...
      await axios.post(
        `${process.env.AUTH_SERVICE_URL || "http://localhost:3001"}/api/auth/email/send`,
        {
          to: userEmail,
          subject: title,
