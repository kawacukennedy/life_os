...
export class MonitoringService {
  ...
  private readonly stripeApiDuration: Histogram<string>;
  private readonly stripeRequestsTotal: Counter<string>;
  private readonly subscriptionEventsTotal: Counter<string>;
  ...
  constructor() {
    ...
    this.stripeRequestsTotal = new Counter({
  ...
  // Stripe API metrics
  recordStripeApiCall(endpoint: string, method: string, duration: number, success: boolean) {
    this.stripeApiDuration
      .labels(endpoint, method)
      .observe(duration);

    this.stripeRequestsTotal
      .labels(endpoint, method, success.toString())
      .inc();
  }

  // Subscription event metrics
  recordSubscriptionEvent(eventType: string, status: string) {
    this.subscriptionEventsTotal.labels(eventType, status).inc();
  }
